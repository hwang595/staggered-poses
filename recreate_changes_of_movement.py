...
class staggered_poses:
	...
	def search_staggered_poses_and_recreate(self):
		...
		plt.subplot(511)
		plt.grid(True)
		plt.plot(self.time_stamp, encoder_deriv_filtered, 'b-', linewidth=2)
		plt.subplot(512)
		plt.grid(True)
	#	plt.plot(self.time_stamp[0:-1], kk, 'b--', linewidth=2)
		plt.plot(self.time_stamp[0:-1], kk_filtered, 'b-', linewidth=2)

		plt.subplot(513)
		plt.grid(True)
		plt.plot(self.time_stamp, force1_x_deriv, 'r-', linewidth=2, label='x')
		plt.plot(self.time_stamp, force1_y_deriv, 'g-', linewidth=2, label='y')
		plt.plot(self.time_stamp, tran_force1_z_std, 'b-', linewidth=2, label='z')
		plt.legend()	
		
		plt.subplot(514)
		plt.grid(True)
		plt.plot(self.time_stamp, force2_x_deriv, 'r-', linewidth=2, label='x')
		plt.plot(self.time_stamp, force2_y_deriv, 'g-', linewidth=2, label='y')
		plt.plot(self.time_stamp, tran_force1_z_std, 'b-', linewidth=2, label='z')
		plt.legend()
	...
	#	plt.plot(self.time_stamp[0:-1], kk, 'b--', linewidth=2)
		...
		plt.grid(True)
		plt.subplot(311)
		plt.plot(self.time_stamp, force1_x_deriv, 'r-', linewidth=2)
		plt.subplot(312)
		plt.plot(self.time_stamp, force1_y_deriv, 'g-', linewidth=2)
		plt.subplot(313)
		plt.plot(self.time_stamp, tran_force1_z_std, 'b-', linewidth=2)		
		
		plt.figure(8)
		plt.title("force sensor 2")
		plt.grid(True)
		plt.subplot(311)
		plt.plot(self.time_stamp, force2_x_deriv, 'r-', linewidth=2)
		plt.subplot(312)
		plt.plot(self.time_stamp, force2_y_deriv, 'g-', linewidth=2)
		plt.subplot(313)
		plt.plot(self.time_stamp, tran_force1_z_std, 'b-', linewidth=2)
		'''
		...
		plt.subplot(311)
		plt.plot(self.time_stamp, force1_x_deriv, 'r-', linewidth=2)
		plt.plot(self.time_stamp, force1_x_filtered, 'y--', linewidth=2)
		plt.subplot(312)
		plt.plot(self.time_stamp, force1_y_deriv, 'g-', linewidth=2)
		plt.plot(self.time_stamp, force1_y_filtered, 'y--', linewidth=2)
		plt.subplot(313)
		plt.plot(self.time_stamp, tran_force1_z_std, 'b-', linewidth=2)
		plt.plot(self.time_stamp, force1_z_filtered, 'y--', linewidth=2)
		...
		plt.subplot(311)
		plt.plot(self.time_stamp, force2_x_deriv, 'r-', linewidth=2)
		plt.plot(self.time_stamp, force2_x_filtered, 'y--', linewidth=2)
		plt.subplot(312)
		plt.plot(self.time_stamp, force2_y_deriv, 'g-', linewidth=2)
		plt.plot(self.time_stamp, force2_y_filtered, 'y--', linewidth=2)
		plt.subplot(313)
		plt.plot(self.time_stamp, tran_force1_z_std, 'b-', linewidth=2)
		plt.plot(self.time_stamp, force2_z_filtered, 'y--', linewidth=2)
		'''
		'''
		plt.show()
		'''
...
#	transformed_pos, transformed_axis_angle = transform_to_robot_coor(pos_viv_coor, quat_viv_coor)
	...
	'''
	raw_pos_x = np.array([a[0] for a in center_traj_x])
	raw_pos_y = np.array([a[1] for a in center_traj_z])
	raw_pos_z = np.array([a[2] for a in center])
	'''
	...
	raw_quat_w = np.array([a[0] for a in raw_quat])
	raw_quat_x = np.array([a[1] for a in raw_quat])
	raw_quat_y = np.array([a[2] for a in raw_quat])
	raw_quat_z = np.array([a[3] for a in raw_quat])
	...
	tran_raw_x = [a[0] for a in tran_raw_pos]
	tran_raw_y = [a[1] for a in tran_raw_pos]
	tran_raw_z = [a[2] for a in tran_raw_pos]

	tran_raw_q_w = [a[0] for a in tran_raw_quat]
	tran_raw_q_x = [a[1] for a in tran_raw_quat]
	tran_raw_q_y = [a[2] for a in tran_raw_quat]
	tran_raw_q_z = [a[3] for a in tran_raw_quat]

	
	tran_x = [a[0] for a in transformed_pos]
	tran_y = [a[1] for a in transformed_pos]
	tran_z = [a[2] for a in transformed_pos]

	tran_q_w = [a[0] for a in transformed_quat]
	tran_q_x = [a[1] for a in transformed_quat]
	tran_q_y = [a[2] for a in transformed_quat]
	tran_q_z = [a[3] for a in transformed_quat]
